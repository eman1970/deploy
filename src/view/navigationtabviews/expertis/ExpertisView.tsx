    ...
    const deleteOne = async (_id: any) => {
        ...
        return (
            <div>
                ...
                <p>{newUser._id}</p>
                {users.map((x: any) => <div><span><i><b>ID: </b>{x._id}</i><br /><b>Username: {x.username}</b><br /><b>Password: </b>{x.password}<br /><b>Age: </b>{x.age}<br /><button onClick={() =>{deleteOne(x._id)}}>Delete</button><hr /></span></div>)}<br />
